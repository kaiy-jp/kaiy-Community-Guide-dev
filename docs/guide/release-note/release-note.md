# ver 0.7.1 
- トップページのレイアウトが大幅に変更されました。
- フロアからフロア検索ができるようになりました。
- フロアから新規フロアを作ることができるようになりました。
- フロア管理画面にセキュリティという項目が実装されました。
- チャット画面で絵文字を使えるようになりました。
- 動画セットボタンと動画クリアボタンが実装されました。
- YouTube動画の再生が終わると、リピートされる機能が実装されました。
- フロアに入っているときに、他のユーザーにハートを送る機能が実装されました。
- チャットの通知機能が実装されました。

# ver 0.7.0 2020/7/31
- フォロー中のフロア画面が実装されました。
- チャットモードが実装されました。
- Vimeoの動画をフロアで再生できるようになりました。
- 入退室音が実装されました。
- カメラとマイクのオンオフ切り替え時のエフェクト音が実装されました。

# ver 0.6.9 2020/7/21
- DJブースはツールという名前に変更されました。それに伴い、レイアウトを修正しました。
- バナーエリアにタイトルを追加できるようになりました。
- フロア管理画面のレイアウトを大きく修正しました。
- バナー管理画面はフロア管理画面からアクセスできるようになりました。
- メール設定画面のレイアウトを修正しました。
- ロール機能が追加されました。

# ver 0.6.8 2020/7/18
- DJブースのレイアウトを調整しました。
- ライブモードsmall、ライブモードlargeを実装しました。
- チャットウィンドウのレイアウトを修正しました。
- フォロー画面のレイアウトと文言を修正しました。
- カメラウィンドウが並び替えできるようになりました。ローカルのみ反映するので、自分のみの設定で変更できます。
- 画面共有のウィンドウがわかりやすくなりました。マウスをホバーするとアイコンが表示されます。
- DJブースのレイアウトを調整しました。
- 編集者権限を持っている人のみ、フロア管理画面でフロアの設定をできるようになりました。
- スーパーユーザー機能が実装しました。
- ステージエリアの画面共有コンポーネントにモーダルを実装しました。

# ver 0.6.7 2020/7/17
- カメラウィンドウに個別ボリュームがつきました。カメラウィンドウの左上にボリュームスライダーが表示されています。
- ボリュームスライダーのスピーカーアイコンを1回押すと20%、2回押すと100％にボリュームが自動セットされます。同じフロアで別々の会話ができるようになりました。
- ローディングのアニメーションが実装されました。
- 招待ページのリンクが無効が正常時に一瞬表示されないように修正しました。
- DJブースの再生と停止ボタンをYouTubeの再生停止ができるようになりました。
- フロアの検索機能が実装されました。

# ver 0.6.6 2020/7/16
- TOPページや招待リンクから、フロア入場する一連の流れを整理し、文言を調整しました。
- 認証フロアにフロアオーナーが入場できないバグを修正しました。
